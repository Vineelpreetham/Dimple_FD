            ...
            {items.map((item, i) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 will-change-transform select-none",
                    isActive
    ...
    <div className="relative h-full w-full flex items-center justify-center">
      {item.imageSrc ? (
        <img
          ...
          style={{
            filter: active ? 'drop-shadow(0 20px 40px rgba(0,0,0,0.5))' : 'drop-shadow(0 10px 20px rgba(0,0,0,0.3))',
          }}
          draggable={false}
          loading="eager"
        />
      ...
      {/* Optional faint glow behind active item */}
      {active && (
        <div 
          className="absolute inset-0 rounded-full blur-3xl -z-10 bg-pink-300/10" 
